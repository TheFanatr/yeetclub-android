        ...
        query.findInBackground((reply, e) -> {
            if (e == null) {
                ...
                if (reply.size() > 1) {
                    ...
                    if (!(String.valueOf(reply.get(1)).isEmpty())) {
                ...
                if (reply.size() > 0) {
                    // Check if reply is not empty
                    if (!(String.valueOf(reply.get(0)).isEmpty())) {

                        // Comment text for first reply object
                        String replyMessage2 = reply.get(0).getString(ParseConstants.KEY_COMMENT_TEXT);
                        ...
                        // Create a list to store the likers of this Comment
                        List<String> likedBy2 = reply.get(0).getList("likedBy");
                        ...
                        String commentId2 = String.valueOf(reply.get(0).getObjectId());

                        // Set click listener for like event
                        holder.likeImage3.setOnClickListener(v -> {
                            v.startAnimation(AnimationUtils.loadAnimation(mContext, R.anim.like_click));
                            createLikeForComment(reply.get(0), position, commentId2);
                        });
    ...
    private void downloadProfilePicture1(ViewHolder holder, ParseObject replyObject) {
        ...
        query.findInBackground((user, e) -> {
            if (e == null) {
                ...
                if (user.get(0).getString(ParseConstants.KEY_AUTHOR_FULL_NAME) != null) {
                    holder.fullName2.setText(user.get(0).getString(ParseConstants.KEY_AUTHOR_FULL_NAME));
                } else {
                    holder.fullName2.setText(R.string.anonymous_fullName);
                }
